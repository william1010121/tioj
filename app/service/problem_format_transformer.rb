...
=end
  ...
  def transform_polygon_to_params(folder_path, zip_file_path= @file, language = @language)
    ...
    assert(description["examplein"].keys.sort == description["exampleout"].keys.sort, "The number of examplein and exampleout should be the same")
  ...
  def poloygon_permit_key
    ["name","legend","input","output","examplein","exampleout", "note","memoryLimit","timeLimit","testdata_pair","maxOutputSize", "group","checker"]
  end
