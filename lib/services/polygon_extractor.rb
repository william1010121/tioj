...
class PolygonExtractor
  ...
  def extract_ploygon_problem
    ...
    raise ArgumentError.new("problem test count mismatch(xml and tests case)") unless description["test_count"] == description["testdata_pair"].length
    description
  end
  ...
  def process_xml(entry,description)
    ...
    description["test_count"] = hash["problem"]["judging"]["testset"]["test_count"].to_i
    raise ArgumentError.new("test count mismatch") unless tests["test"].length == description["test_count"]
  ...
  def process_document(entry,description)
    ...
    if match
      case match[1]
        ...
        when "notes.tex"
          description["note"] = latex_to_md.call( entry.get_input_stream.read )
        when /^example\.\d+$/
